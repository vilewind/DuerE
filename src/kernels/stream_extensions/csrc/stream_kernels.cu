...
template <typename T> __device__ double to_double(T src) {
...
template <typename T> __device__ T from_double(double src) {
  if constexpr (std::is_same_v<T, double>) {
    ...
    static_assert(!std::is_same_v<T, T>, "Unsupported type for from_double");
  }
}

template <typename T> __device__ float to_float(T src) {
...
template <typename T> __device__ T from_float(float src) {
...
template <typename T> T *get_ptr(torch::Tensor &src) {
  if constexpr (std::is_same_v<T, double> || std::is_same_v<T, float>) {
...
template <typename T> const T *get_ptr(const torch::Tensor &src) {
  if constexpr (std::is_same_v<T, double> || std::is_same_v<T, float>) {
    return src.data_ptr<T>();
  } else if constexpr (std::is_same_v<T, cuDoubleComplex>) {
...
__global__ void fused_rope_qk_forward(
    const int64_t b, const int64_t total_seq_len, const int64_t h,
    const int64_t d, const int64_t stride_b, const int64_t stride_s,
    const int64_t stride_h, const int64_t stride_d, const int64_t o_stride_b,
    const int64_t o_stride_s, const int64_t o_stride_h,
    const int64_t o_stride_d, const scalar_t_0 *__restrict__ q_src,
    const scalar_t_0 *__restrict__ k_src,
    const cuDoubleComplex *__restrict__ freq, const int64_t *__restrict__ grid,
    scalar_t_1 *__restrict__ q_dst, scalar_t_1 *__restrict__ k_dst) {
...
#pragma unroll
  for (int32_t d_id = threadIdx.x; d_id < half_d; d_id += blockDim.x) {
    ...
    int32_t base_offset_src = blockIdx.y * stride_b + blockIdx.x * stride_s +
                              (d_id * 2 + 0) * stride_d;
    int32_t base_offset_dst = blockIdx.y * o_stride_b +
                              blockIdx.x * o_stride_s +
                              (d_id * 2 + 0) * o_stride_d;
...
#pragma unroll
    for (int32_t h_id = threadIdx.y; h_id < h; h_id += blockDim.y) {
      ...
      if (blockIdx.x < total_video_len) {
        ...
        q_dst[offset_dst] = from_double<scalar_t_1>(cuCreal(q_dst_value));
        q_dst[offset_dst + o_stride_d] =
            from_double<scalar_t_1>(cuCimag(q_dst_value));

        k_dst[offset_dst] = from_double<scalar_t_1>(cuCreal(k_dst_value));
        ...
        q_dst[offset_dst + o_stride_d] =
            from_double<scalar_t_1>(to_double(q_src[offset_src + stride_d]));

        k_dst[offset_dst] =
            from_double<scalar_t_1>(to_double(k_src[offset_src]));
        k_dst[offset_dst + o_stride_d] =
            from_double<scalar_t_1>(to_double(k_src[offset_src + stride_d]));
      }
    }
  }
}
...
template <typename scalar_t_0, typename scalar_t_1>
void fused_rope_qk_forward_call(
    const int64_t b, const int64_t total_seq_len, const int64_t h,
    const int64_t d, const int64_t stride_b, const int64_t stride_s,
    const int64_t stride_h, const int64_t stride_d, const int64_t o_stride_b,
    const int64_t o_stride_s, const int64_t o_stride_h,
  ...
  fused_rope_qk_forward<<<grid_dims, block_dims, shared_mem_size, stream>>>(
      b, total_seq_len, h, d, stride_b, stride_s, stride_h, stride_d,
      o_stride_b, o_stride_s, o_stride_h, o_stride_d, q_src, k_src, freq, grid,
      q_dst, k_dst);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
}

void fused_rope_qk_fwd(const torch::Tensor &q_src, const torch::Tensor &k_src,
                       const torch::Tensor &freq, const torch::Tensor &grid,
                       torch::Tensor &q_dst, torch::Tensor &k_dst) {
  c10::ScalarType src_type = q_src.scalar_type();
  c10::ScalarType dst_type = q_dst.scalar_type();
  TORCH_CHECK(src_type == c10::ScalarType::Double ||
                  src_type == c10::ScalarType::Float ||
                  src_type == c10::ScalarType::BFloat16 ||
                  src_type == c10::ScalarType::Half,
              "expected the dtype of the src tensor is torch.float64 or "
              "torch.float32 or torch.bfloat16 or torch.float16");
  TORCH_CHECK(dst_type == c10::ScalarType::Double ||
                  dst_type == c10::ScalarType::Float ||
                  dst_type == c10::ScalarType::BFloat16 ||
                  dst_type == c10::ScalarType::Half,
              "expected the dtype of the dst tensor is torch.float64 or "
              "torch.float32 or torch.bfloat16 or torch.float16");
  TORCH_CHECK(
  ...
  const int64_t b = q_src.size(0);             // batch size
  const int64_t total_seq_len = q_src.size(1); // seq len
  const int64_t h = q_src.size(2);             // head num
  const int64_t d = q_src.size(3);             // d_model / head num
  ...
  DISPATCH_TYPES(src_type, dst_type, "forward_kernel_qk_partial_call",
                 fused_rope_qk_forward_call(
                     b, total_seq_len, h, d, stride_b, stride_s, stride_h,
                     stride_d, o_stride_b, o_stride_s, o_stride_h, o_stride_d,
                     get_ptr<scalar_t_0>(q_src), get_ptr<scalar_t_0>(k_src),
                     get_ptr<cuDoubleComplex>(freq), get_ptr<int64_t>(grid),
                     get_ptr<scalar_t_1>(q_dst), get_ptr<scalar_t_1>(k_dst)););
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("fused_rope_qk_fwd", &fused_rope_qk_fwd,
        "Fused Rotary Positional QK Embedding -- Forward.", py::arg("q_src"),
        py::arg("k_src"), py::arg("freq"), py::arg("grid"), py::arg("q_dst"),
        py::arg("k_dst"), py::call_guard<py::gil_scoped_release>());
}